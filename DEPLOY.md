...
코드 수정이 끝나면 아래 명령을 실행하세요. **배포할까요?** 라고 물어보고, `y` 입력 시 커밋·**푸시까지 자동** 진행 후 Vercel이 자동 배포합니다.
...
- 변경 파일 목록이 보이고, `배포할까요? (y/n):` 에 **y** 입력 → 커밋 메시지 입력(비우면 "chore: 배포 업데이트") → **자동으로 푸시** → Vercel 자동 배포
- **n** 입력 시 취소

### (선택) 커밋만 해도 자동 푸시

`git commit` 후에 매번 `git push`를 치지 않으려면, 아래를 **한 번만** 실행해 두세요. 이후에는 커밋만 하면 자동으로 푸시됩니다.

```bash
npm run install-auto-push
```

이렇게 하면 `git add` → `git commit -m "..."` 까지 한 뒤, **푸시는 자동**으로 실행됩니다.
